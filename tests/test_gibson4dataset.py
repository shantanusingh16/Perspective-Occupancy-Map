...
class TestGibson4Dataset(TestCase):
    ...
    def test_colordiff_train(self):
        rgb0, sem0, pom0, bev0, proj_bev0 = self.train_dataset[0]

        rgb1, sem1, pom1, bev1, proj_bev1 = self.train_dataset[0]
        ...
        self.assertEqual(torch.isclose(proj_bev0, proj_bev1).all(), True)

        rgb1, sem1, pom1, bev1, proj_bev1 = self.train_dataset[1]
        ...
        self.assertEqual(torch.isclose(proj_bev0, proj_bev1).all(), False)

    def test_colordiff_test(self):
        rgb0, sem0, pom0, bev0, proj_bev0 = self.test_dataset[0]

        rgb1, sem1, pom1, bev1, proj_bev1 = self.test_dataset[0]
        ...
        rgb1, sem1, pom1, bev1, proj_bev1 = self.test_dataset[1]
        self.assertEqual(torch.isclose(rgb0, rgb1).all(), False)
        self.assertEqual(torch.isclose(sem0, sem1).all(), False)
        self.assertEqual(torch.isclose(pom0, pom1).all(), False)
        self.assertEqual(torch.isclose(bev0, bev1).all(), False)
