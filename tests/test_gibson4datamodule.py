...
class TestGibson4Dataset(TestCase):
    ...
    def setUp(self) -> None:
        ...
        self.dm = Gibson4DataModule(self.dataconfig.data_dir, self.dataconfig.split_dir, self.batch_size)
        self.dm.setup()
           
    def test_checksizes(self):

        train_len = len(self.dataconfig.train_files)
        val_len = len(self.dataconfig.val_files)
    ...
    def test_batchsize(self):
        ...
        self.assertEqual(len(train_batch), 5)  # RGB, SEM, POM, BEV, PROJBEV
        ...
        self.assertEqual(len(test_batch), 5)  # RGB, SEM, POM, BEV, PROJBEV
